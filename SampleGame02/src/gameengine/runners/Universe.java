					...
					for(int i=0;i<collisionObjects.size();i++){
						...
						for(int j=1;j<collisionObjects.size();j++){
							...
							if(obj.getBoundingBox().intersects(obj2.getBoundingBox())){
								...
								if(rect1.intersectsLine(rect2.x,rect2.y,rect2.x+rect2.width,rect2.y)){
									if(obj2.isMobableByCollision())
										obj2.move(rect2.x,rect1.y-rect2.height);
									else
										obj.move(rect1.x,rect2.y-rect1.height);
									continue;
								}
								//object1 is been touched by object2 underneath
								if(rect2.intersectsLine(rect1.x,rect1.y,rect1.x+rect1.width,rect1.y)){
									if(obj2.isMobableByCollision())
									   obj2.move(rect2.x,rect1.y+rect1.height);
									else 
										obj.move(rect1.x,rect2.y+rect2.height);
									continue;
								}
								//object1 is been touched by object2 on the right
								if(rect1.intersectsLine(rect2.x,rect2.y,rect2.x,rect2.y+rect2.height)){
									if(obj2.isMobableByCollision())   
										obj2.move(rect1.x+rect1.width,rect2.y);
									else 
										obj.move(rect2.x+rect2.width,rect1.y);
									continue;
								}
								//object1 is been touched by object2 on the left
								if(rect1.intersectsLine(rect2.x+rect2.width,rect2.y,rect2.x+rect2.width,rect2.y+rect2.height)){
									if(obj2.isMobableByCollision())
										obj2.move(rect1.x-rect2.width,rect2.y);
									else
